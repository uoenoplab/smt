...
main(int argc, char **argv)
{
	...
	if (tls && smt_enable_test_tls(fd, true) != 0) {
		close(fd);
		return (1);
	}
	...
	for (int i = 0; i < exchanges; i++) {
		...
		union smt_recv_cmsgbuf recv_control;
		struct iovec iov;
		...
		n = recvmsg(fd, &msg, SMT_RECVMSG_REQUEST);
		if (n < 0) {
		...
		if (extract_recv_args(&msg, &recv_args) != 0) {
			fprintf(stderr,
			    "missing SMT control message in request\n");
			close(fd);
			return (1);
		}
		...
		{
			union smt_send_cmsgbuf send_control;
