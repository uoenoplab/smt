...
#include "smt_test_tls.h"

#define SMT_CMSG_TYPE		1
...
union smt_send_cmsgbuf {
	struct cmsghdr hdr;
	unsigned char buf[CMSG_SPACE(sizeof(struct smt_sendmsg_args))];
};

union smt_recv_cmsgbuf {
	struct cmsghdr hdr;
	unsigned char buf[CMSG_SPACE(sizeof(struct smt_recvmsg_args))];
};
...
static int
extract_recv_args(struct msghdr *msg, struct smt_recvmsg_args *out)
{
	struct cmsghdr *cmsg;
...
send_one(int fd, struct sockaddr_in *peer, char *message,
    uint64_t completion_cookie)
{
	union smt_send_cmsgbuf control;
	struct smt_sendmsg_args send_args;
	struct iovec iov;
	struct msghdr msg;
	struct cmsghdr *cmsg;
	ssize_t sent;
...
recv_one(int fd, int quiet, const char *expected_message,
    uint64_t *completion_cookie)
{
	char data_buf[SMT_MAX_MESSAGE_LENGTH];
	union smt_recv_cmsgbuf control;
	struct iovec iov;
	struct msghdr msg;
	struct sockaddr_in from;
	struct smt_recvmsg_args recv_args;
	ssize_t n;
	size_t expected_len;
	...
	n = recvmsg(fd, &msg, SMT_RECVMSG_RESPONSE);
	if (n < 0) {
	...
	if (extract_recv_args(&msg, &recv_args) != 0) {
		fprintf(stderr, "missing SMT control message in response\n");
		return (-1);
	}
...
main(int argc, char **argv)
{
	...
	if (strlen(message) > SMT_MAX_MESSAGE_LENGTH) {
	...
	if (tls && smt_enable_test_tls(fd, false) != 0) {
		close(fd);
		free(completed);
		return (1);
	}
