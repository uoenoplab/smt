...
static int
client_exchange(int fd, const struct smt_simple_config *config,
    const struct sockaddr_in *server, const void *request, void *response,
    struct smt_bench_histogram *histogram, int *preheat)
{
	...
	if (smt_bench_now(&sent_at) != 0)
		return (-1);
	sent = smt_bench_send_request(fd, server, request,
	...
	received = smt_bench_recv(fd, SMT_BENCH_RECV_RESPONSE, response,
	...
	if (smt_bench_now(&received_at) != 0)
		return (-1);
	rtt_us = smt_bench_elapsed_us(&sent_at, &received_at);
	if (*preheat > 0) {
		(*preheat)--;
	} else if (smt_bench_hist_add(histogram, rtt_us,
...
static int
client_print_results(struct smt_bench_histogram *histogram,
    const struct timespec *start, const struct timespec *end)
{
	...
	benchmark_time = smt_bench_elapsed_seconds(start, end);
	...
	printf("  \"average_rtt_us\": %.2f,\n",
	    smt_bench_hist_average(histogram));
	printf("  \"average_stddev_rtt_us\": %.2f,\n",
	    smt_bench_hist_stddev(histogram));
	printf("  \"p50_median_rtt_us\": %.2f,\n",
	    smt_bench_hist_percentile(histogram, 50.0));
	printf("  \"p95_rtt_us\": %.2f,\n",
	    smt_bench_hist_percentile(histogram, 95.0));
	printf("  \"p99_rtt_us\": %.2f\n",
	    smt_bench_hist_percentile(histogram, 99.0));
	printf("}\n");
	printf("--- RESULT ---\n");
	return (0);
}
...
main(int argc, char **argv)
{
	...
	struct smt_bench_histogram histogram;
	...
	parse_result = smt_simple_parse_args(argc, argv, false, &config);
	if (parse_result != 0) {
		smt_simple_usage(argv[0], false);
		return (parse_result > 0 ? 0 : 2);
	}
	if (smt_bench_resolve_ipv4(config.server_address, config.port,
	...
	preheat = SMT_BENCH_PREHEAT_RPCS;
	result = 0;
	if (smt_bench_now(&benchmark_start) != 0) {
	...
	if (smt_bench_now(&benchmark_end) != 0) {
		perror("clock_gettime");
		result = 1;
