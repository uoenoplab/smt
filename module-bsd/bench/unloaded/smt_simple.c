...
void
smt_simple_usage(const char *program, bool server)
{
	fprintf(stderr, "Usage: %s [OPTIONS]\n", program);
	fprintf(stderr, "Options compatible with Linux simple_%s:\n",
	    server ? "server" : "client");
	fprintf(stderr, "  -r, --proto homa|smt\n");
	if (server) {
...
static int
smt_simple_parse_u64(const char *text, uint64_t *value)
{
	char *end;
	uintmax_t parsed;
...
static int
smt_simple_parse_duration(const char *text, double *duration)
{
	char *end;
...
int
smt_simple_parse_args(int argc, char **argv, bool server,
    struct smt_simple_config *config)
{
	...
	while ((ch = getopt_long(argc, argv, "r:a:p:l:n:vqB:d:TPh",
		...
		switch (ch) {
		case 'r':
			if (smt_bench_parse_protocol(optarg,
				&config->protocol) != 0)
				return (-1);
			break;
		case 'a':
			...
			break;
		case 'p':
			if (smt_bench_parse_u16(optarg, &config->port) != 0)
				return (-1);
			break;
		case 'l':
			if (smt_bench_parse_sizes(optarg,
				&config->request_size,
		...
		case 'B':
			...
			break;
		case 'd':
			if (smt_simple_parse_duration(optarg,
				&config->duration) != 0)
				return (-1);
			break;
		case 'T':
	...
	if (offset_text == NULL)
		offset_text = getenv("SMT_BENCH_PIN_CORE_OFFSET");
	if (offset_text != NULL &&
	    smt_bench_parse_int(offset_text, 0, 65535,
		&config->pin_offset) != 0)
		return (-1);
	offset_text = getenv("HOMA_ECHO_PIN_CORE_DISABLE");
	if (offset_text != NULL) {
		if (smt_bench_parse_int(offset_text, 0, INT_MAX,
			&disable_pin) != 0)
			return (-1);
		config->pin = disable_pin == 0;
	}

	if (optind != argc || config->protocol == SMT_BENCH_PROTO_UNSET ||
...
void
smt_simple_print_config(const struct smt_simple_config *config, bool server,
    const char *server_numeric)
{
	...
	printf("{\n");
	printf("  \"protocol\": \"%s\",\n",
	    smt_bench_protocol_name(config->protocol));
	if (server) {
		printf("  \"listen\": \"%s:%u\",\n", config->bind_address,
		    config->port);
	} else {
	...
	printf("  \"verbose_level\": \"%s\"\n",
	    smt_bench_verbose_name(config->verbose));
	printf("}\n");
	printf("%s\n\n", banner);
}

int
smt_simple_open_socket(const struct smt_simple_config *config, bool server)
{
	int fd;

	fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_SMT);
	if (fd < 0)
		return (-1);
	if (smt_bench_bind_socket(fd, config->bind_address,
		server ? config->port : 0) != 0 ||
	    ((config->protocol == SMT_BENCH_PROTO_SMT ||
	      config->force_tls) &&
		smt_bench_enable_tls(fd, server) != 0)) {
		close(fd);
		return (-1);
	}
	return (fd);
