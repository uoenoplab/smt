...
static int set_tls_crypto_info(void *crypto_info_send_ptr, void *crypto_info_read_ptr, int tls13, int server, int key_size, int alter) {
    unsigned char client_key_hardcode[32] = {0x8D, 0xD2, 0x30, 0xA7, 0x7A, 0x05, 0xEB, 0x71, 0x15, 0x91, 0x29, 0xBC, 0xBC, 0xF6, 0x42, 0x30, 0x8D, 0xD2, 0x30, 0xA7, 0x7A, 0x05, 0xEB, 0x71, 0x15, 0x91, 0x29, 0xBC, 0xBC, 0xF6, 0x42, 0x30};
    unsigned char client_iv_hardcode[12] = {0x87, 0xC6, 0x35, 0xC8, 0x17, 0x87, 0xDE, 0x4A, 0x88, 0x1D, 0xD2, 0xD5};
    unsigned char server_key_hardcode[32] = {0x6C, 0xCF, 0x62, 0xFF, 0x4B, 0xE6, 0x14, 0x85, 0xD8, 0xBA, 0x29, 0xFE, 0x2E, 0x84, 0x7A, 0x7F, 0x6C, 0xCF, 0x62, 0xFF, 0x4B, 0xE6, 0x14, 0x85, 0xD8, 0xBA, 0x29, 0xFE, 0x2E, 0x84, 0x7A, 0x7F};
    unsigned char server_iv_hardcode[12] = {0xB9, 0xFA, 0x55, 0x83, 0xD5, 0x8F, 0x85, 0x18, 0xFF, 0xA6, 0x3E, 0x66};

    unsigned char client_key_hardcode_alter[32] = {0xE0, 0x47, 0x0E, 0x9F, 0x09, 0x48, 0x27, 0x9B, 0xE9, 0x8B, 0xF3, 0xB1, 0x18, 0x58, 0x4F, 0xB6, 0xE0, 0x47, 0x0E, 0x9F, 0x09, 0x48, 0x27, 0x9B, 0xE9, 0x8B, 0xF3, 0xB1, 0x18, 0x58, 0x4F, 0xB6};
    unsigned char client_iv_hardcode_alter[12] = {0x39, 0x00, 0x8E, 0x2E, 0xFE, 0x3E, 0x37, 0x81, 0xCF, 0x71, 0x66, 0x69};
    unsigned char server_key_hardcode_alter[32] = {0x6A, 0x0D, 0xF9, 0x6B, 0xDD, 0x28, 0x08, 0xF8, 0x3C, 0x4D, 0x70, 0x07, 0x43, 0x4C, 0xE0, 0x38, 0x6A, 0x0D, 0xF9, 0x6B, 0xDD, 0x28, 0x08, 0xF8, 0x3C, 0x4D, 0x70, 0x07, 0x43, 0x4C, 0xE0, 0x38};
    unsigned char server_iv_hardcode_alter[12] = {0xBE, 0x9D, 0x1C, 0xEA, 0xF8, 0xE9, 0x54, 0x23, 0x32, 0x8E, 0xD4, 0x20};

    unsigned char *local_key, *local_iv, *remote_key, *remote_iv;
    ...
    if (key_size == 128) {
        ...
        if (tls13) {
            ...
            memcpy(crypto_info_send->iv, &local_sequence_number, TLS_CIPHER_AES_GCM_256_IV_SIZE);
        }
        ...
        memcpy(crypto_info_send->rec_seq, &local_sequence_number, TLS_CIPHER_AES_GCM_256_REC_SEQ_SIZE);
        ...
        if (tls13) {
            ...
            memcpy(crypto_info_read->iv, &local_sequence_number, TLS_CIPHER_AES_GCM_256_IV_SIZE);
        }
...
// exmaple for create a SMT socket
int main() {
