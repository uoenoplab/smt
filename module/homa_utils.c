...
int homa_init(struct homa *homa)
{
	...
	strncpy(homa->smt_hardware_interface, "enp1s0f0np0", sizeof(homa->smt_hardware_interface) - 1);
	homa->smt_replay_guard = 0;
		...
		const sockaddr_in_union *dest)
{
	...
	// Delay get hoamls ctx here to save a socket lock call
	crpc->smt_ctx = smt_set_rpc_context(crpc, ipv6_to_ipv4(dest_addr_as_ipv6),
			dest->in6.sin6_port);
	if (crpc->smt_ctx) {
		if ((err = smt_set_rpc_offload_context(crpc)))
			goto error;
	}
		...
		struct sk_buff *skb)
{
	...
	// Delay get hoamls ctx here to save a socket lock call
	srpc->smt_ctx = smt_set_rpc_context(srpc, ipv6_to_ipv4(*source),
			h->common.sport);
	if (srpc->smt_ctx && hsk->homa->smt_replay_guard &&
			smt_replay_guard_check_duplicate(
				(struct smt_context *) srpc->smt_ctx,
				srpc->id)) {
		homa_sock_unlock(hsk);
		err = -EALREADY;
		goto error;
...
//		if (smt_ip_id(skb) == 0 && smt_gso_offset(skb) == 0) {
//			atomic_or(RPC_PKTS_READY, &srpc->flags);
//			homa_rpc_handoff(srpc);
//		}
	}
...
#define BATCH_MAX 20
	...
	release:
		...
		for (i = 0; i < num_rpcs; i++) {
			 ...
			 * RPC yet.
			 */
			//printk("%s reaped %llu\n", __FUNCTION__, rpcs[i]->id);
			smt_destroy_rpc(rpcs[i]);
