...
bool client_smt = false;
bool server_iovec = false;
bool server_smt = false;
...
void print_help(const char *name)
{
	printf("Usage: cp_node [command]\n\n"
		...
		"    --smt          Use smt (encrypted communication) instead of homa, \n"
		"                      for TCP, kTLS is also used with this option is set\n"
        ...
        , threads()
{
	...
	if (server_smt) {
		status = smt_setsockopt_wrapper(fd, 0, 0, 1, 0);
		if (status < 0) {
			printf("FATAL: error in smt_setsockopt: %s\n",
				strerror(errno));
			exit(1);
		}
	}
...
void tcp_server::accept(int epoll_fd)
{
	...
	if (server_smt) {
		if (tcpktls_setsockopt_wrapper(fd, 1, 0) < 0) {
			printf("FATAL: error in tcpktls_setsockopt_wrapper: %s\n",
				strerror(errno));
			exit(1);
		}
	}
        ...
        , sending_thread()
{
	...
	if (client_smt) {
		status = smt_setsockopt_wrapper(fd, 0, 0, 0, 0);
		if (status < 0) {
			printf("FATAL: error in smt_setsockopt: %s\n",
				strerror(errno));
			exit(1);
        ...
        , sending_thread()
{
	...
	for (uint32_t i = 0; i < server_addrs.size(); i++) {
		...
		if (client_smt) {
			if (tcpktls_setsockopt_wrapper(fd, 0, 0) < 0) {
				printf("FATAL: error in tcpktls_setsockopt_wrapper: %s\n",
					strerror(errno));
				exit(1);
			}
