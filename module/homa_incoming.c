		...
		int incoming)
{
	...
	msgin->smt_gsoseg_offset = 0;
	msgin->smt_decrypt_offset = 0;
	msgin->smt_gsoseg_skb = &msgin->packets;
	msgin->smt_decrypt_skb = &msgin->packets;
	msgin->smt_max_pkt_data = 0;
	msgin->smt_nextgsoseg_length = 0;
		...
		struct homa_lcache *lcache, int *delta)
{
	...
	// Note: we delay acks hanlding for both Homa and SMT to
	// smt_copy_to_user and homa_copy_to_user for as
	// 1. ack attached to segment header is encrypted in the SMT
	//  (segment header)
	// 2. It is no possible to differentiate between skb for Homa or
	// SMT before we know which rpc this skb belongs to.
	...
	smt_prdbg_int("%s lcache %px skb %px\n", __FUNCTION__, lcache, skb);
		...
		if (!homa_is_client(id)) {
			...
			if (h->type == DATA) {
				...
				rpc = homa_rpc_new_server(hsk, &saddr,
						(struct data_header *) h, skb);
				if (IS_ERR(rpc)) {
					long err = PTR_ERR(rpc);
					if (err == -EALREADY) {
						smt_prerr_int(
							"%s dropping replay id %llu\n",
							__FUNCTION__, id);
		...
		struct homa_lcache *lcache, int *delta)
{
	...
	int old_remaining;
	int rpc_handoff = 0;
	struct sk_buff* skb_gso_tail = NULL;
	bool try_find_gsoseg = false;
	...
	if (rpc->msgin.total_length < 0) {
		...
		*delta += rpc->msgin.incoming;
		...
		/* Set smt_max_pkt_data for first data packet */
		if (rpc->smt_ctx)
			rpc->msgin.smt_max_pkt_data =
	...
	if (rpc->smt_ctx)
		smt_add_packet(rpc, skb, &try_find_gsoseg);
	else
		homa_add_packet(rpc, skb);
	*delta -= old_remaining - rpc->msgin.bytes_remaining;

	smt_prinf_int("%s: try_find_gsoseg %d\n", __FUNCTION__, try_find_gsoseg);

	if (rpc->smt_ctx && try_find_gsoseg) {
		rpc_handoff = smt_find_gsoseg(rpc) && (!(atomic_read(&rpc->flags) & RPC_PKTS_READY));
	} else if (!rpc->smt_ctx && (ntohl(h->seg.offset) == rpc->msgin.copied_out)) {
		rpc_handoff = !(atomic_read(&rpc->flags) & RPC_PKTS_READY);
	}

	// smt_prdbg_int("%s lcache %px lcache->rpc %px spin_is_locked(rpc->lock) %d\n",
	// __FUNCTION__, lcache, lcache->rpc, spin_is_locked(rpc->lock));

	if (rpc_handoff) {
		atomic_or(RPC_PKTS_READY, &rpc->flags);
		homa_sock_lock(rpc->hsk, "homa_data_pkt");
		smt_prinf_int("%s homa_rpc_handoff\n", __FUNCTION__);
		...
		struct homa_sock *hsk)
{
	...
	if (rpc->msgout.next_xmit_offset < rpc->msgout.granted) {
		...
		if (rpc->smt_ctx)
			smt_resend_data(rpc, ntohl(h->offset),
...
void homa_unknown_pkt(struct sk_buff *skb, struct homa_rpc *rpc)
{
	...
	if (homa_is_client(rpc->id)) {
		if (rpc->state == RPC_OUTGOING) {
			...
			if (rpc->smt_ctx)
				smt_resend_data(rpc, 0, rpc->msgout.next_xmit_offset,
...
found_rpc:
		...
		if (rpc) {
			...
			if (!rpc->error) {
				if (rpc->smt_ctx)
					rpc->error = smt_copy_to_user(rpc);
				else
			...
			tt_record2("homa_wait_for_message copied_out %d total_length %d",
	...
	tt_record("homa_wait_for_message done");
	if (blocked)
