...
#endif
MODULE_AUTHOR("Tianyi Gao and John Ousterhout");
MODULE_DESCRIPTION("Homa transport protocol with SMT addon");
MODULE_VERSION("0.01");
...
static struct ctl_table homa_ctl_table[] = {
	{
		...
		.data = &homa_data.smt_hardware_interface,
	...
	},
	{
		...
		.data		= &homa_data.smt_hardware_state_threshold,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec
	},
	{
		.procname	= "smt_replay_guard",
		.data		= &homa_data.smt_replay_guard,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec
	},
...
static __u16 header_lengths[] = {
	sizeof32(struct data_header) - sizeof32(struct data_segment), // to allow trailer only
...
static int __init homa_load(void) {
	...
	printk(KERN_NOTICE "Homa/SMT module loading\n");
	printk(KERN_NOTICE "Homa/SMT structure sizes: data_header %u, "
...
static void __exit homa_unload(void) {
	printk(KERN_NOTICE "Homa/SMT module unloading\n");
	exiting = true;
		...
		unsigned int optlen)
{
	...
	if (level == SOL_TLS)
		return smt_setsockopt(hsk, optname, optval, optlen);
...
int homa_sendmsg(struct sock *sk, struct msghdr *msg, size_t length) {
	...
	if (!args.id) {
		...
		result = (rpc->smt_ctx) ?
			smt_message_out_init(rpc, &msg->msg_iter, 1)
			: homa_message_out_init(rpc, &msg->msg_iter, 1);
