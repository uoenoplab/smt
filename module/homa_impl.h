...
#define task_sigpending(xxx) mock_task_sigpending
extern int mock_task_sigpending;
...
struct homa_message_in {
	...
	/**
	 * @smt_decrypt_offset: Amount of data for this mesage taht
	 * has been decrypted
	 */
	int smt_decrypt_offset;
	struct sk_buff_head *smt_decrypt_skb;

	int smt_gsoseg_offset;
	int smt_nextgsoseg_length; // include segment header
	int smt_nextgsoseg_received; // include segment header
	struct sk_buff_head *smt_gsoseg_skb;
...
#define RPC_ACKING_SMT     32 // handling ackes after decryption

#define RPC_CANT_REAP (RPC_COPYING_FROM_USER | RPC_COPYING_TO_USER \
		| RPC_HANDING_OFF | RPC_DECRYPTING | RPC_ACKING_SMT)
...
#define HOMA_RPC_MAGIC 0xdeadbeef
	...
	/**
	 * @smt_ctx: pointer for smt context associated with this rpc
	 *  bound with 4-tuples
	 */
	void *smt_ctx;
	void *smt_rpc_offload_ctx_tx;
	void *smt_rpc_offload_ctx_rx;
};
...
// #define homa_rpc_lock(rpc) {printk("%s homa_rpc_lock", __FUNCTION__); if (!spin_trylock_bh(rpc->lock)) homa_rpc_lock_slow(rpc);}
...
struct homa_sock {
	...
	struct hlist_head smt_ctx_buckets[HOMA_SERVER_RPC_BUCKETS];
	void *smt_reuse_ctx;
};
...
struct homa {
	...
	/**
	 * @smt_replay_guard: nonzero enables message id tracking for replay
	 * attack prevention
	 */
	int smt_replay_guard;
