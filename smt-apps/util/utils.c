...
ssize_t homa_init_recv_buffer(int sockfd, size_t *recv_buf_size,
  ...
  *recv_buf_region = mmap(NULL, *recv_buf_size, PROT_READ | PROT_WRITE,
  ...
  buf_args.start = (uint64_t)(uintptr_t)*recv_buf_region;
  buf_args.length = *recv_buf_size;

  if (setsockopt(sockfd, IPPROTO_HOMA, SO_HOMA_RCVBUF, &buf_args,
                 sizeof(buf_args)) < 0) {
    fprintf(stderr, "Error in setsockopt(SO_HOMA_RCVBUF): %s\n", strerror(errno));
    goto mmap;
  }
