#ifndef _SDTP_TEST_TLS_H_
#define _SDTP_TEST_TLS_H_
...
#define SDTP_TXTLS_ENABLE	31
#define SDTP_RXTLS_ENABLE	32
...
static int
sdtp_enable_test_tls(int fd, bool server)
{
	...
	if (setsockopt(fd, IPPROTO_SDTP, SDTP_TXTLS_ENABLE, &tx,
	    sizeof(tx)) < 0) {
		perror("setsockopt(SDTP_TXTLS_ENABLE)");
		return (-1);
	}
	if (setsockopt(fd, IPPROTO_SDTP, SDTP_RXTLS_ENABLE, &rx,
	    sizeof(rx)) < 0) {
		perror("setsockopt(SDTP_RXTLS_ENABLE)");
		return (-1);
	}
